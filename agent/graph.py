...
class PipelineState(TypedDict, total=False):
    api_key: str  # 접속자가 입력한 Anthropic API 키
    csv_text: str
...
def run_pipeline(api_key: str, csv_text: str, memo_text: Optional[str] = None) -> Dict[str, Any]:
    """전체 파이프라인을 실행하고 최종 state를 반환."""
    return GRAPH.invoke(
        {"api_key": api_key, "csv_text": csv_text, "memo_text": memo_text, "revision_count": 0}
    )


def stream_pipeline(api_key: str, csv_text: str, memo_text: Optional[str] = None):
