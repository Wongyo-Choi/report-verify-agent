...
with st.expander("📖 사용법 — 어떻게 쓰나요?"):
    st.markdown(
        """
**이 도구가 하는 일**
측정값 CSV(와 현장 메모)를 올리면, 여러 AI가 **분석 → 보고서 초안 작성 → 오류 검증**을 자동으로 수행함.
사람은 마지막에 결과만 확인하고 승인하면 됨.

**4단계로 따라하기**
1. **왼쪽 사이드바**에서 LLM 공급자(Claude·ChatGPT·Gemini)와 모델을 고르고, **본인 API 키**를 입력함.
2. **측정값 CSV**(필수)와 **현장 메모 txt**(선택)를 올림. 자료가 없으면 **"📂 샘플 데이터로 실행"** 버튼을 누름.
3. **"🚀 파이프라인 실행"**을 누르면 가드레일 → 분석 → 작성 → 검증 진행 상황이 차례로 표시됨.
4. 나온 **보고서 초안**과 **검증 결과(PASS/FAIL)**를 확인한 뒤, **"✔️ 승인"** 하거나 **"⬇️ .md 다운로드"** 함.

**CSV 형식** — 다음 열이 있어야 함: `항목, 측정값, 단위, 기준, 판정`
(예: `진동 가속도, 3.2, m/s^2, 2.5, 불합격`)

**API 키 안내** — 키는 서버에 저장되지 않고 입력한 키로만 호출됨. 따라서 **요금은 본인 키로 청구**됨.
공급자별 키 발급 링크는 사이드바에 있음.
"""
    )
...
"AI에게 일을 잘 시키는 법"이 아니라, **AI가 안전하고 일관되게 일하도록 작업장(하네스)을 설계**하는 방식.
이 도구는 그 부품들을 코드로 고정해, 누가 실행해도 같은 품질의 보고서가 나오게 함.
...
검증자가 **FAIL**을 내면 작성자에게 자동으로 되돌려 **재작성**함(최대 1회). 마지막 승인은 **사람**이 함(= 검문소).

**왜 검증자를 따로 두나?**
자기가 쓴 보고서를 자기가 검토하면 오류를 놓치기 쉬움(자기평가 편향). 작성 과정을 모르는 **독립 검증자**가
백지에서 원본과 대조하므로 그럴듯하지만 틀린 값을 잡아냄.

**환각(없는 내용 지어내기) 억제**
- 작업 수칙(RULES.md)을 모든 단계 프롬프트에 주입 → "자료에 있는 값만 사용" 규칙 강제
- 분석·검증 출력을 **구조화 스키마(JSON)** 로 강제해 형식 일탈을 막음

**멀티 LLM 지원**
`litellm`으로 추상화 → Anthropic(Claude)·OpenAI(ChatGPT)·Google(Gemini) 등 **어떤 공급자 키든** 동작.
