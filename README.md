...
## 공급자/키 모델 — 접속자 본인 키, 멀티 LLM

LLM 호출을 `litellm`으로 추상화해 **Anthropic(Claude)·OpenAI(ChatGPT)·Google(Gemini)** 중
원하는 공급자를 사이드바에서 고르고 본인 키를 입력한다. (모델명도 자유 입력 가능)

API 키는 **서버에 저장하지 않으며**, 입력한 키로만 호출한다. → 누가 이 URL을 써도
**과금은 그 사람 본인 키로** 청구됨. (앱 소유자 키가 공용으로 쓰이지 않으므로 안심하고 공개 가능)
